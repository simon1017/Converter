import tkinter as tk
import customtkinter as ctk
from tkinter import filedialog, messagebox, scrolledtext
import os
import converter
import translator as tl

class TextEditorWindow:
    def __init__(self, root, text):
        self.root = root
        self.root.title("Text Editor")

        self.text_editor = scrolledtext.ScrolledText(self.root, wrap=tk.WORD)
        self.text_editor.pack(expand=True, fill='both', padx=5, pady=5)
        self.text_editor.insert(tk.END, text)

        button_frame = ctk.CTkFrame(self.root)
        button_frame.pack()

        self.save_button = ctk.CTkButton(button_frame, text="Save", command=self.save_and_close)
        self.save_button.pack(side=tk.LEFT, padx=5, pady=5)

        self.close_button = ctk.CTkButton(button_frame, text="Close", command=self.close_window)
        self.close_button.pack(side=tk.LEFT, padx=5, pady=5)

        self.edited_text = None

    def save_and_close(self):
        self.edited_text = self.text_editor.get("1.0", tk.END)
        self.root.destroy()

    def close_window(self):
        self.root.destroy()


class PDFToWavConverterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("PDF to WAV Converter")

        # Initialize converter
        self.converter = converter.Converter()

        self.translator = tl.MyTranslator()

        # Create input widgets
        self.create_input_widgets()

    def create_input_widgets(self):
        # PDF path
        self.pdf_path_label = ctk.CTkLabel(self.root, text="PDF path:")
        self.pdf_path_label.grid(row=0, column=0, padx=5, pady=5)
        self.pdf_path_input = ctk.CTkEntry(self.root)
        self.pdf_path_input.grid(row=0, column=1, padx=5, pady=5)
        self.pdf_path_button = ctk.CTkButton(self.root, text="Select PDF", command=self.select_pdf_path)
        self.pdf_path_button.grid(row=0, column=2, padx=5, pady=5)

        # save folder path
        self.save_folder_label = ctk.CTkLabel(self.root, text="Save path:")
        self.save_folder_label.grid(row=1, column=0, padx=5, pady=5)
        self.save_folder_input = ctk.CTkEntry(self.root)
        self.save_folder_input.grid(row=1, column=1, padx=5, pady=5)
        self.save_folder_button = ctk.CTkButton(self.root, text="Select folder", command=self.select_save_folder)
        self.save_folder_button.grid(row=1, column=2, padx=5, pady=5)

        # WAV filename
        self.wav_name_label = ctk.CTkLabel(self.root, text="WAV filename:")
        self.wav_name_label.grid(row=2, column=0, padx=5, pady=5)
        self.wav_name_input = ctk.CTkEntry(self.root)
        self.wav_name_input.grid(row=2, column=1, padx=5, pady=5)

        # Start page
        self.page_start_label = ctk.CTkLabel(self.root, text="Start page:")
        self.page_start_label.grid(row=3, column=0, padx=5, pady=5)
        self.page_start_input = ctk.CTkEntry(self.root)
        self.page_start_input.insert(0, "0")
        self.page_start_input.grid(row=3, column=1, padx=5, pady=5)

        # End page
        self.page_end_label = ctk.CTkLabel(self.root, text="End page:")
        self.page_end_label.grid(row=4, column=0, padx=5, pady=5)
        self.page_end_input = ctk.CTkEntry(self.root)
        self.page_end_input.insert(0, "0")
        self.page_end_input.grid(row=4, column=1, padx=5, pady=5)

        # speech rate scale
        self.page_end_label = ctk.CTkLabel(self.root, text="Speech rate: w/m")
        self.page_end_label.grid(row=5, column=0, padx=5, pady=5)
        s_r = int(self.converter.get_speech_rate())
        self.speech_rate_value = tk.IntVar(self.root)
        self.speech_rate_slider = ctk.CTkSlider(self.root, from_=s_r-200, to=s_r+200, orientation="Horizontal", command=self.on_slider_move, variable=self.speech_rate_value)
        self.speech_rate_slider.set(s_r)
        self.speech_rate_slider.grid(row=5, column=1, columnspan=1, padx=5, pady=5)
        self.speech_rate_value_label = ctk.CTkLabel(self.root, textvariable= self.speech_rate_value)
        self.speech_rate_value_label.grid(row=5, column=2, padx=5, pady=5)

        # Create a StringVar to store the selected option
        self.voice_menu_var = tk.StringVar(self.root)
        # Generate initial options
        options = self.converter.get_voices()
        names = []
        for index, voice in enumerate(options):
            #if index < 20:
            names.append(voice.name)
        self.voice_menu_var.set(names[14])
        # voice menu and label
        self.page_end_label = ctk.CTkLabel(self.root, text="Voices:")
        self.page_end_label.grid(row=6, column=0, padx=5, pady=5)
        self.voice_menu = ctk.CTkOptionMenu(self.root, variable=self.voice_menu_var, values= names)
        self.voice_menu.grid(row=6, column=1, padx=5, pady=5)

        # speak button
        self.convert_button = ctk.CTkButton(self.root, text="Speak", command= lambda: self.converter.voice_speak(self.voice_menu_var.get().strip()))
        self.convert_button.grid(row=6, column=2, padx=5, pady=5)

        # translation checkbox
        self.use_translation_bool = tk.BooleanVar(value=False)
        self.use_translation_checkbox = ctk.CTkCheckBox(self.root, variable=self.use_translation_bool, onvalue=True, offvalue=False, text="use translation")
        self.use_translation_checkbox.grid(row=7, column=0, padx=5, pady=5)

        # translate from language
        self.translate_from_Label = ctk.CTkLabel(self.root, text="From: ")
        self.translate_from_Label.grid(row=8, column=0, padx=5, pady=5)

        langs = self.translator.get_langs()
        print(f"type is: {type(langs)}")
        self.from_lang = tk.StringVar(self.root)
        self.from_lang.set(langs[21])
        self.translate_from_menu = ctk.CTkOptionMenu(master=self.root, variable=self.from_lang, values=langs)
        self.translate_from_menu.grid(row=8, column=1, padx=5, pady=5)

        # translate to language
        self.translate_to_Label = ctk.CTkLabel(self.root, text="To: ")
        self.translate_to_Label.grid(row=9, column=0, padx=5, pady=5)

        self.to_lang = tk.StringVar(self.root)
        self.to_lang.set(langs[91])
        self.translate_to_menu = ctk.CTkOptionMenu(master=self.root, variable=self.to_lang, values=langs)
        self.translate_to_menu.grid(row=9, column=1, padx=5, pady=5)

        # Convert button
        self.convert_button = ctk.CTkButton(self.root, text="Convert", command=self.open_text_editor)
        self.convert_button.grid(row=10, column=1, padx=5, pady=5)

    def select_pdf_path(self):
        pdf_path = filedialog.askopenfilename(filetypes=[("PDF Files", "*.pdf")])
        self.pdf_path_input.insert(0, pdf_path)

    def select_save_folder(self):
        save_path = filedialog.askdirectory()
        self.save_folder_input.insert(0, save_path)

    def on_slider_move(self, slider_value):
        self.speech_rate_value.set(slider_value)
        print(f"slider value: {self.speech_rate_value.get()}")
        self.converter.set_speech_rate(self.speech_rate_value.get())


    def open_text_editor(self):
        # Get input values
        pdf_path = self.pdf_path_input.get().strip()
        save_folder = self.save_folder_input.get().strip()
        wav_name = self.wav_name_input.get().strip()
        page_start = int(self.page_start_input.get().strip())
        page_end = int(self.page_end_input.get().strip())
        speech_rate = self.speech_rate_slider.get()
        voice_name = str(self.voice_menu_var.get().strip())

        # Set converter parameters
        self.converter.set_pdf_path(pdf_path)
        self.converter.set_save_folder(save_folder)
        self.converter.set_wav_name(wav_name)
        self.converter.set_page_start(page_start)
        self.converter.set_page_end(page_end)
        self.converter.set_speech_rate(speech_rate)
        self.converter.set_active_voice(voice_name)

        
        if not pdf_path:
            messagebox.showerror("Error", "PDF path is required.")
            return
        
        if not wav_name:
            messagebox.showerror("Error", "WAV filename is required.")
            return
        
        if not save_folder:
            messagebox.showerror("Error", "Save folder is required.")
            return
        
        if not pdf_path or not wav_name:
            messagebox.showerror("Error", "PDF path and WAV filename are required.")
            return

        if not os.path.exists(pdf_path):
            messagebox.showerror("Error", "PDF file not found.")
            return
        
        if page_start <= 0 or page_end <= 0:
            messagebox.showerror("Error", "pages has to be greater than 0.")
            return

        if page_start > page_end:
            messagebox.showerror("Error", "Start page greater than End page")
            return
        
        #print(type(text))
        #print(f"use translation: {self.use_translation_bool.get()}")
        if self.use_translation_bool.get():
            src = self.translator.get_codes(lang=self.from_lang.get())
            dest = self.translator.get_codes(lang=self.to_lang.get())
            self.converter.set_src(src)
            self.converter.set_dest(dest)
        
        text = self.converter.read_pdf(self.use_translation_bool.get())
        root_editor = tk.Tk()
        text_editor_window = TextEditorWindow(root_editor, text)
        root_editor.wait_window(text_editor_window.root)
        edited_text = text_editor_window.edited_text
        #print(f"closed window, \nedited text:\n{edited_text}")
        self.convert_pdf_to_wav(edited_text)

    def convert_pdf_to_wav(self, edited_text):
        if edited_text is not None:
            self.converter.convert(edited_text)
            messagebox.showinfo("Success", f"Conversion completed. WAV file saved.")

def main():
    root = tk.Tk()
    app = PDFToWavConverterApp(root)
    root.mainloop()

if __name__ == "__main__":
    main()
